...
export function getLandingModulo(segmentIndex: number): number {
  const center = getSegmentCenterAngle(segmentIndex);
...
export function getTargetRotation(
  ...
  const landingMod = getLandingModulo(segmentIndex);
...
export function getSegmentAtPointer(rotation: number, segmentCount: number): number {
  ...
  return Math.floor(pointerAngle / SEGMENT_ANGLE) % segmentCount;
}
